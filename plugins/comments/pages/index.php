...
if ($func == 'delete' && $id > 0) {
    ...
    $sql = rex_sql::factory();
    //$sql->setDebug();
    $sql->setTable(rex::getTablePrefix() . 'podcastmanager_comments');
    $sql->setWhere(['id' => $id]);

    try {
        $sql->delete();
        $success = rex_i18n::msg('podcastmanager_deleted');
...
if ($func == 'status' && $id > 0) {
    ...
    try {
        $sql->setQuery('UPDATE '. rex::getTablePrefix() . 'podcastmanager_comments SET status = IF(status=1, 0, 1) WHERE id = '.$id.'');
...
if ($func == "") {
    ...
    $list = rex_list::factory(''
            . 'SELECT * '
            . 'FROM ' . rex::getTablePrefix() . 'podcastmanager_comments '
...
$str = "<a class=\"no-wrap\" href=\"index.php?page=podcastmanager/comments&func=status&id=###id###\"><span class=\"rex-online\"><i class=\"rex-icon rex-icon-online\"></i> ".rex_i18n::msg("comment_online")."</span></a>";
else
$str = "<a class=\"no-wrap\" href=\"index.php?page=podcastmanager/comments&func=status&id=###id###\"><span class=\"rex-offline\"><i class=\"rex-icon rex-icon-offline\"></i> ".rex_i18n::msg("comment_offline")."</span></a>";
return $str;'
        )
    );
    ...
    $list->addLinkAttribute('deleteComment', 'data-confirm', rex_i18n::msg('podcastmanager_delete_question') . ' ?');
    $list->setColumnFormat('deleteComment', 'custom', function ($params) {
        $list = $params['list'];
        return $list->getColumnLink('deleteComment', '<i class="rex-icon rex-icon-delete"></i> ' . rex_i18n::msg('podcastmanager_delete'));
    });
    ...
    $form = rex_form::factory(rex::getTablePrefix() . 'podcastmanager_comments', '', 'id = ' . $id);
    ...
    $article_result->getArray('SELECT `title` FROM ' . rex::getTablePrefix() . 'podcastmanager WHERE `pid` = '. $article_id );
