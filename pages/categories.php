...
if ($func == 'delete' && $category_id > 0) {
    $sql = rex_sql::factory();
    //$sql->setDebug();
    $sql->setTable(rex::getTablePrefix() . 'podcastmanager_categories');
...
if ($func == "") {
    if (count(rex_clang::getAll(1))>1) {
        ...
        foreach(rex_clang::getAll(1) as $key => $lang) {
            ...
            echo '<a '.$active.' href="index.php?page=podcastmanager/categories&clang_id='.$lang->getId().'"><i class="rex-icon rex-icon-online"></i> '. $lang->getName() .'</a>';
        }
    ...
    $list = rex_list::factory('SELECT pid,id,name,clang_id FROM ' . rex::getTablePrefix() . 'podcastmanager_categories WHERE clang_id = '.$clang_id.' ORDER BY name ASC', 25);
    ...
    $list->setColumnFormat('deleteCategory', 'custom', function ($params) {
        $list = $params['list'];
        return $list->getColumnLink('deleteCategory', '<i class="rex-icon rex-icon-delete"></i> ' . rex_i18n::msg('podcastmanager_delete'));
    });
    ...
    $form = rex_form::factory(rex::getTablePrefix() . 'podcastmanager_categories', '', 'pid = ' . $category_id);
