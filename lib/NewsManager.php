...
class NewsManager
{
    ...
    public function __construct()
    {
        ...
        $this->tpl = new Template(rex_path::addonData('podcastmanager') . 'views/');

        $urlData = UrlGenerator::getData();

        if (($urlData) && ($urlData->urlParamKey == 'podcastmanager')) {
            $this->news_id_parameter = UrlGenerator::getId();
        }

        if (($urlData) && ($urlData->urlParamKey == 'podcastmanager_category')) {
    ...
    private function initializeArticleObject($result, $lang_id)
    {
        ...
        $newsArticle->setNewsmanager_category_id($result->getValue('podcastmanager_category_id'));
        ...
        $newsArticle->setUrl(rex_getUrl('', $lang_id, ['podcastmanager' => $result->getValue('id')]));
    ...
    public function getArticleObjectList($limit)
    {
        ...
        if ($category_id != 0) {
            $AND_clause .= 'AND `podcastmanager_category_id` LIKE \'%|' . $category_id . '|%\' ';
        };

        $allNewsArticleObjects = [];

        $query = 'SELECT SQL_CALC_FOUND_ROWS * '
                . 'FROM `' . rex::getTablePrefix() . 'podcastmanager` '
                . 'WHERE `status` = 1 '
                . $AND_clause
    ...
    public function printSingleView($newsArticle)
    {
        ...
        if ($newsArticle->getId() != null) {
            ...
            $output .= $this->tpl->render($suggestions, array(
                'title' => rex_i18n::rawMsg('podcastmanager_error_headline'),
                'error' => rex_i18n::rawMsg('podcastmanager_error_no_post')
            ));
        }
        return $output;
    }
    ...
    public function printCategoryMenu()
    {
        ...
        $query = 'SELECT *'
                . 'FROM `' . rex::getTablePrefix() . 'podcastmanager_categories` '
        ...
        while ($result->hasNext()) {
            ...
            $category_menu .= '<li ' . $current . '><a href="' . rex_getUrl('', '', ['podcastmanager_category' => $result->getValue('id')]) . '">' . $result->getValue('name') . '</a></li>' . PHP_EOL;
    ...
    public static function generateRssFeeds()
    {
        ...
        if (count(rex_clang::getAll()) >= 1) {
            foreach (rex_clang::getAll() as $key => $lang) {
                if ($lang->isOnline()) {
                    $podcastmanager->generateRssFeed($lang);
                }
            }
        }
    }
